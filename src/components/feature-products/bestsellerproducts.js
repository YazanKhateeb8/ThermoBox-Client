...
import { fetchMostSoldProducts } from '../../servicesApi/ProductsApi';
import './BestSellerProducts.css';
import AddShoppingCartIcon from '@mui/icons-material/AddShoppingCart';
import { Link } from 'react-router-dom'
import { addProductToCart  } from '../../servicesApi/CartApi';

export default function BestSellerProducts( { user } ) {
  ...
  useEffect(() => {
    const getMostSoldProducts = async () => {
      const fetchedMostSoldProducts = await fetchMostSoldProducts();
      setMostSoldProducts(fetchedMostSoldProducts);
    };
    getMostSoldProducts();
  }, []);
  ...
  useEffect(() => {
    ...
    const handleCarousel = () => {
      if (!isHovered) {
        const nextProductIndex = (currentPage + 1) % Math.ceil(mostSoldProducts.length / 4);
  ...
  }, [mostSoldProducts, currentPage, isHovered]);
